...
<body>
    ...
    <div class="formtitle"><span>类别信息</span></div>
		<form action="${basePath}category" method="post">
<%--			这个隐藏域用来调用CategoryServlet--%>
			<input type="hidden" name="method" value="updateCategoryByCid"/>
<%--	这个隐藏域用来存储要修改商品类别唯一标识cid --%>
			<input type="hidden" name="cid" value="${category.cid}"/>
	    <ul class="forminfo">
		    <li><label>类别名称</label><input name="cname" type="text" value="${category.cname}" class="dfinput" /><i>类别名称不能超过30个字符</i></li>s
			<li>
				<label>是否启用</label>
			    <cite>
